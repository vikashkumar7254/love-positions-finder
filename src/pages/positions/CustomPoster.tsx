        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 space-y-6 sm:space-y-8">
          ...
          <div className="text-center mb-8 sm:mb-12">
            <div className="flex flex-col sm:flex-row items-center justify-center gap-2 sm:gap-4 mb-4 sm:mb-6">
              <Palette className="w-8 h-8 sm:w-12 sm:h-12 text-pink-300" />
              ...
              <Card variant="elegant" className="bg-black/20 border-white/10">
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <CardTitle className="text-xl sm:text-2xl text-white">Your Journey</CardTitle>
                    <div className="flex gap-2">
                      {canRevealToday() ? (
                        ...
                        >
                          ...
                          Reveal Day {journeyProgress.currentDay}
                        </Button>
                      ) : (
                        <Button
                          variant="outline"
                      ...
                      <Button
                        ...
                        className="text-red-300 border-red-500/30 hover:bg-red-500/20 hover:text-red-200"
                      >
                        <X className="w-4 h-4" />
                        Reset
                      </Button>
                ...
                <CardContent>
                  {/* Poster Design */}
                  <div ref={posterRef} className="bg-gradient-to-br from-black/40 to-black/20 rounded-xl p-6 border border-white/10">
                    {/* Poster Header */}
                    <div className="text-center mb-6">
                      <input
                        ...
                        className="text-2xl sm:text-3xl font-bold text-white bg-transparent border-none outline-none text-center w-full"
                      ...
                      <input
                        ...
                        className="text-sm text-white/70 bg-transparent border-none outline-none text-center w-full mt-1"
                    ...
                    <div className="grid grid-cols-4 gap-4 mb-6 text-center">
                      <div>
                        <div className="text-lg font-bold text-white">{journeyProgress.currentDay > 7 ? 7 : journeyProgress.currentDay}</div>
                        <div className="text-xs text-white/60">Current Day</div>
                      </div>
                      <div>
                        <div className="text-lg font-bold text-pink-300">{revealedSlots}</div>
                        <div className="text-xs text-white/60">Revealed</div>
                      </div>
                      <div>
                        <div className="text-lg font-bold text-purple-300">{completionPercentage}%</div>
                        <div className="text-xs text-white/60">Completion</div>
                      </div>
                      <div>
                        <div className="text-lg font-bold text-red-300">{daysSinceStart}</div>
                  ...
                  <div className="flex flex-wrap gap-3 mt-6 justify-center">
                    <Button variant="romantic" className="flex items-center gap-2">
                      <Download className="w-4 h-4" />
                      Download Poster
                    </Button>
                    <Button variant="outline" className="flex items-center gap-2 text-blue-300 border-blue-500/30 hover:bg-blue-500/20 hover:text-blue-200">
                      <Save className="w-4 h-4" />
                      Save Design
                    </Button>
                    <Button variant="outline" className="flex items-center gap-2 text-green-300 border-green-500/30 hover:bg-green-500/20 hover:text-green-200">
                ...
                <CardContent>
                  <div className="space-y-4">
                    ...
                    <div className="text-center p-4 bg-gradient-to-br from-pink-500/10 to-purple-500/10 rounded-lg border border-pink-500/20">
                      {canRevealToday() ? (
                        <>
                          <Calendar className="w-8 h-8 text-pink-300 mx-auto mb-2 animate-pulse" />
                          <div className="text-white font-semibold mb-1">Day {journeyProgress.currentDay} Ready!</div>
                          <div className="text-white/70 text-sm mb-3">Click the pulsing card above to reveal today's position</div>
                          <Button
                            variant="romantic"
                            onClick={revealTodaysPosition}
                        ...
                        <>
                          ...
                          <div className="text-white font-semibold mb-1">Journey Complete!</div>
                          <div className="text-white/70 text-sm">You've revealed all 7 positions. Congratulations!</div>
                        </>
                      ) : (
                        <>
                          <Clock className="w-8 h-8 text-yellow-300 mx-auto mb-2" />
                          <div className="text-yellow-300 font-semibold mb-1">Come Back Tomorrow</div>
                          <div className="text-yellow-200/80 text-sm">You've already revealed today's position</div>
                        </>
                      )}
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 text-center">
                      ...
                      <div className="p-3 bg-black/20 rounded-lg">
                        <div className="text-lg font-bold text-purple-300">{daysSinceStart}</div>
                        <div className="text-xs text-white/60">Days Since Start</div>
                      </div>
                    </div>
